    ...
    xmlhttp.onreadystatechange = function() {
        if (xmlhttp.readyState == XMLHttpRequest.DONE) {   // XMLHttpRequest.DONE == 4
            if (xmlhttp.status == 200) {
                document.querySelector("#game-page-info-options").innerHTML = this.response;
                document.querySelector("#update").addEventListener("click", updateF);
                document.querySelector("#delete").addEventListener("click", removeF);
            }
...
if (summary.innerHTML.length > 300){
    summary.innerHTML = summary.innerHTML.substr(0, 300)+"<span id='dododot'>...</span><span id='more' hidden>"+summary.innerHTML.substr(301, summary.innerHTML.length-1)+"</span>&nbsp&nbsp<a id='more-btn'>View more</a>";
